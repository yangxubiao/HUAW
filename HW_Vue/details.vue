...
export default {
    ...
    reduct(){
       if(this.count==1)
         ...
         car(){    
            if(localStorage.getItem('user')==0||localStorage.getItem('user')==null){
                   this.alert=true;
            }else{
              if(this.count>=1){
                 var uid=JSON.parse(localStorage.getItem('user')).uid;
                   ...
                   var url="/product/car?pid="+pid+"&lid="+lid+"&q="+q+"&uid="+uid+"&sum="+this.count;
...
ul{
  ...
  #confirm{
      position:absolute;top:50%;left:50%;width:500px;height:100px;background-color:#f10215;z-index:1000;margin-left:-250px;margin-top:-50px;font-size:20px;color:#fff;
  }
    #confirm button{
      position:absolute;top:70%;left:90%;width:3rem;height:2rem;
    }
  #count{
